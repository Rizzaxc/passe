...
class NetworkSelectionScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> attemptPop() async {
      if (!networkController.hasUncommittedChanges) {
      ...
      if (discard == true && context.mounted) {
        networkController.discardChanges();
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) attemptPop();
      },
      child: FScaffold(
        header: FHeader.nested(
          title: Text('profile.networkLabel'.tr()),
          prefixes: [FHeaderAction.back(onPress: attemptPop)],
        ),
