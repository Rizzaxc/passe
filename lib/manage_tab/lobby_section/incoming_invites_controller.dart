...
/// A pending `invite` befriend record where the current user is the target —
/// a lobby captain inviting them in. Accepting flips `status` to `accepted`;
/// a DB trigger (`lobby_befriend_accepted_trigger_fn`) then adds membership.
///
/// The sibling `pair` interaction (user↔user, created a 2-person lobby on
/// accept) is **retired** — see schema/friendship.sql. Friendship is the
/// supported user↔user relationship now, so this only reads `invite` rows.
class IncomingInvite {
  final String id;
  final String inviterUsername;
  final String inviterTag;

  /// The lobby the user is invited into.
  final String? lobbyName;

  /// The lobby's sport, used for the sport chip.
  final Sport? sport;
...
class IncomingInvitesController extends _$IncomingInvitesController {
  ...
  Future<List<IncomingInvite>> build() async {
    ...
    return (response as List).map((row) {
      ...
      final sportId = lobby?['sport_id'] as int?;
