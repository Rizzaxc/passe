-- Adds each row's author (and, for payment_request rows, each payee)
-- generatedAvatar to lobby_feed_data's output.
--
-- The lobby feed client (lib/manage_tab/lobby_section/activity/feed.dart)
-- was rendering every author/payee as a plain initial-letter circle because
-- this RPC only ever returned username/user_id, never the details needed to
-- resolve a real avatar (see PUserAvatar / profile_tab/CLAUDE.md's
-- generatedAvatar overload). The wall_post-sourced 'photo' branch already
-- carried the full author_details, so it's untouched here.

-- `author_generated_avatar` must be appended at the end of the output list,
-- not inserted after `author_username` — CREATE OR REPLACE FUNCTION on a
-- RETURNS TABLE function only allows adding new output columns at the end;
-- reordering/inserting mid-list requires DROP FUNCTION first.
